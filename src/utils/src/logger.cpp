...
void getCharSeparatedStringList(const char *cString, std::vector<std::string> &lines, char separator) {
    lines.clear();
    std::string line;
...
void Logger::print(Logger::PRINT_COLOR color, const char *fmt, ...) {
    ...
    //Write to terminal
    if (color == Logger::RED)			printf("%s%s%s", ANSI_COLOR_RED, pBuffer, ANSI_COLOR_RESET);
    else if (color == Logger::GREEN)	printf("%s%s%s", ANSI_COLOR_GREEN, pBuffer, ANSI_COLOR_RESET);
    else if (color == Logger::YELLOW)	printf("%s%s%s", ANSI_COLOR_YELLOW, pBuffer, ANSI_COLOR_RESET);
    else if (color == Logger::BLUE)		printf("%s%s%s", ANSI_COLOR_BLUE, pBuffer, ANSI_COLOR_RESET);
    else if (color == Logger::MAGENTA)	printf("%s%s%s", ANSI_COLOR_MAGENTA, pBuffer, ANSI_COLOR_RESET);
    else if (color == Logger::CYAN)		printf("%s%s%s", ANSI_COLOR_CYAN, pBuffer, ANSI_COLOR_RESET);
...
void Logger::consolePrint(const char *fmt, ...) {
    ...
    for (unsigned int i = 0; i < newLines.size(); i++)
        consoleOutput.push_back(ConsoleText{ newLines[i] , Eigen::Vector3d::Ones() });
