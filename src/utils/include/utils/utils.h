	...
	while (!feof(fp)) {
		if(fgets(line, nChars, fp)){
			if ((int)strlen(line) >= nChars)
				std::cout << "The input file contains a line that is longer than the buffer - not allowed" << std::endl;
		}
		else
...
/**
	This method returns a std::vector of char pointers that correspond to the addressed
