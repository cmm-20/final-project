...
public:
    ...
    void processMouseMove(MouseState ms, KeyboardState ks) {
        ...
        if (ms.lButtonPressed && ks[GLFW_KEY_LEFT_ALT] == false)
            processLeftMouseMovement(ms.mouseMoveX, ms.mouseMoveY);
        if (ms.rButtonPressed || (ms.lButtonPressed && ks[GLFW_KEY_LEFT_ALT] == true))
            processRightMouseMovement(ms.mouseMoveX, ms.mouseMoveY);
    }
    ...
    //xpos and ypos represent points in screen coordinates (e.g. where the mouse cursor is); rayOrigin and rayDirection will represent quantities in world coordinates...
    void getRayFromScreenCoordinates(double xpos, double ypos, P3D& rayOrigin, V3D& rayDirection){
        //this method assumes the correct viewport is set when this method is called - can get slightly tricky when multiple sub windows are used...
        glm::vec4 viewportParams;
        glGetFloatv(GL_VIEWPORT, &viewportParams[0]);

        //0 and 1 are here the near and far z planes in normalized coordinates (e.g. right before projection to screen coordinates)
