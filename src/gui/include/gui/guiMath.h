...
*/
    ...
    ImGuizmo::Manipulate(glm::value_ptr(cameraView), glm::value_ptr(cameraProjection), mCurrentGizmoOperation, mCurrentGizmoMode, glm::value_ptr(matrix), NULL, useSnap ? &snap[0] : NULL, boundSizing ? bounds : NULL, boundSizingSnap ? boundsSnap : NULL);
}
...
//    orientation = Quaternion(AngleAxisd(RAD(matrixRotation[0]), V3D(1, 0, 0))) *
//                  Quaternion(AngleAxisd(RAD(matrixRotation[1]), V3D(0, 1, 0))) *
//                  Quaternion(AngleAxisd(RAD(matrixRotation[2]), V3D(0, 0, 1)));

    // don't understand why it's this order. In the reconstruct matrix method, it seems to be the other way around...
    orientation = Quaternion(AngleAxisd(RAD(matrixRotation[2]), V3D(0, 0, 1))) *
                  Quaternion(AngleAxisd(RAD(matrixRotation[1]), V3D(0, 1, 0))) *
