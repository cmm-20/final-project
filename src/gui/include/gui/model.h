...
class Model {
public:
    std::vector<Mesh::TextureMap> material_textures;	// stores all the textures loaded so far, optimization to make sure textures aren't loaded more than once.
    ...
    void draw(const Shader &shader, const V3D &color, const glm::mat4 &transform) const;
...
public:
    bool hitByRay(
            const P3D &r_o, const V3D &r_v,
            P3D &hitPoint, double &t, V3D &n) const;

    bool hitByRay(
     const P3D &r_o, const V3D &r_v) const;

    bool hitByRay(
     const P3D &r_o, const V3D &r_v, double& t) const;

    bool hitByRay(
     const P3D &r_o, const V3D &r_v, P3D &hitPoint) const;

    bool hitByRay(
            const P3D &r_o, const V3D &r_v, P3D &hitPoint, V3D &hitNormal) const;
